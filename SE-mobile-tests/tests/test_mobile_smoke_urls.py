...
ФАЙЛ_URL = Path(__file__).resolve().parents[1] / "config" / "urls_mobile.txt"
...
def _ascii_slug(текст: str) -> str:
    slug = "".join(
        символ.lower() if ("a" <= символ.lower() <= "z" or "0" <= символ <= "9") else "_"
        for символ in текст
    )
    return "_".join(часть for часть in slug.split("_") if часть)


def _имя_динамического_теста(url: str, device_name: str) -> str:
    хеш = hashlib.sha1(f"{device_name}|{url}".encode("utf-8")).hexdigest()[:12]
    return f"test_mobile_smoke_{_ascii_slug(device_name)}_{хеш}"
...
def _выполнить_смоук_проверку(url: str, device_name: str, page: Page) -> None:
    уникальный_id = _стабильный_allure_id(device_name, url)
    allure.dynamic.id(уникальный_id)
    allure.dynamic.label("ALLURE_ID", уникальный_id)
    allure.dynamic.label("AS_ID", уникальный_id)
...
def _создать_динамический_тест(url: str, device_name: str) -> object:
    @pytest.mark.timeout(90)
    @pytest.mark.mobile_device(device_name)
    def _тест(device_name: str, page: Page) -> None:
        _выполнить_смоук_проверку(url, device_name, page)

    имя_теста = _имя_динамического_теста(url, device_name)
    _тест.__name__ = имя_теста
    _тест.__qualname__ = имя_теста
    _тест.__doc__ = f"Смоук-проверка | {device_name} | {url}"
    return _тест


def _зарегистрировать_динамические_тесты() -> None:
    url_список = загрузить_url_из_файла(ФАЙЛ_URL)
    if not url_список:
        raise pytest.UsageError(f"Файл со ссылками пуст: {ФАЙЛ_URL}")

    for url in url_список:
        for device_name in УСТРОЙСТВА:
            имя_теста = _имя_динамического_теста(url, device_name)
            if имя_теста in globals():
                raise RuntimeError(f"Дублируется имя теста: {имя_теста}")
            globals()[имя_теста] = _создать_динамический_тест(url, device_name)


_зарегистрировать_динамические_тесты()
